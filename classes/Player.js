...
class Player {
  ...
  constructor(AC, params = {}) {
    ...
    console.log("New Synth", Player.AC.currentTime);
    ...
    this.nodes.volume = Player.AC.createGain();
    this.setParam("volume");

    this.nodes.adsr = Player.AC.createGain();

    this.nodes.filter = Player.AC.createBiquadFilter();
    this.nodes.filter.type = "lowpass";
    this.setParam("filterFreq");
    this.setParam("filterQ");

    this.nodes.delay = Player.AC.createDelay(Player.MAX_ECHO_DURATION);
    this.nodes.feedback = Player.AC.createGain();
    this.setParam("echoTime");
    this.setParam("echoFeedback");

    this.nodes.compressor = Player.AC.createDynamicsCompressor();

    this.nodes.analyser = Player.AC.createAnalyser();
    ...
    this.nodes.analyser.connect(Player.AC.destination);
  ...
  setParam = (param, value = this.params[param]) => {
    ...
    switch (param) {
      ...
      case "filterFreq":
        ...
        break;
      case "filterQ":
        this.nodes.filter.Q.value = value * Player.MAX_FILTER_Q;
        break;
      case "echoTime":
        this.nodes.delay.delayTime.value = value * Player.MAX_ECHO_DURATION;
        break;
      case "echoFeedback":
  ...
  noteOn = (freq, t = 0) => {
    ...
    const ct = Player.AC.currentTime;
  ...
  timePlus = (secs) => {
    return Player.AC.currentTime + secs;
  };
